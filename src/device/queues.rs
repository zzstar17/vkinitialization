...
pub unsafe fn retrieve_single_queues(
  ...
  #[cfg(feature = "graphics_family")]
  let graphics = Queue {
    handle: unsafe { device.get_device_queue(queue_families.graphics.index, 0) },
  ...
  #[cfg(not(feature = "graphics_family"))]
  let compute = Queue {
    handle: unsafe { device.get_device_queue(queue_families.compute.index, 0) },
  ...
  let mut next_non_specialized_queue = || {
    if non_specialized_i < c_infos[0].queue_count {
      let r = Some(Queue {
        handle: unsafe {
          device.get_device_queue(c_infos[0].queue_family_index, non_specialized_i)
        },
  ...
  let compute = if let Some(compute_f) = queue_families.compute {
    Queue {
      handle: unsafe { device.get_device_queue(compute_f.index, 0) },
  ...
  let transfer = if let Some(transfer_f) = queue_families.transfer {
    Queue {
      handle: unsafe { device.get_device_queue(transfer_f.index, 0) },
...
impl QueueFamilies {
  ...
  pub fn get_from_physical_device(
    ...
    for (i, props) in properties.into_iter().enumerate() {
      ...
      let cur_graphics = props.queue_flags.contains(vk::QueueFlags::GRAPHICS) && graphics.is_none();
      #[cfg(feature = "surface")]
      let cur_graphics =
        cur_graphics && unsafe { surface.supports_queue_family(physical_device, i)? };
      if cur_graphics {
        graphics = family;
        continue;
      }
